...
import org.eclipse.aether.RepositorySystemSessionHelper;
...
import com.hubspot.snapshots.AcceleratorHelper;
        ...
        implements UpdateCheckManager, Service
{
  ...
  {
    disableSnapshotNormalization(session);

    if ( check.getLocalLastUpdated() != 0
            && !isArtifactUpdatedRequired( session, check.getLocalLastUpdated(), check ) )
    {
  ...
  public void checkMetadata( RepositorySystemSession session, UpdateCheck<Metadata, MetadataTransferException> check )
  {
    ...
    if ( check.getLocalLastUpdated() != 0
            && !isMetadataUpdatedRequired( session, check.getLocalLastUpdated(), check ) )
    {
      if ( logger.isDebugEnabled() )
      {
        logger.debug( "Skipped remote request for " + check.getItem()
  ...
  private boolean isMetadataUpdatedRequired( RepositorySystemSession session, long lastModified, UpdateCheck<Metadata, ?> check )
  {
    ...
    boolean updateRequired = updatePolicyAnalyzer.isUpdatedRequired( session, lastModified, check.getPolicy() );
    if ( updateRequired )
    {
      boolean acceleratorShortCircuit = AcceleratorHelper.INSTANCE.shouldSkipUpdate(
              session.getLocalRepository(),
              check.getItem(),
              check.getRepository()
      );
      if ( acceleratorShortCircuit )
      {
        if ( logger.isDebugEnabled() )
        {
          logger.debug("Snapshot accelerator short-circuited, skipping metadata update for " + check.getItem());
        }
        return false;
      }
    }
  ...
  private void disableSnapshotNormalization(RepositorySystemSession session) {
    if (session instanceof AbstractForwardingRepositorySystemSession) {
      session = RepositorySystemSessionHelper.getSession((AbstractForwardingRepositorySystemSession) session);
    }

    if (session.getConfigProperties().containsKey("aether.artifactResolver.snapshotNormalization")) {
      return;
    }

    if (!DefaultRepositorySystemSession.class.equals(session.getClass())) {
      throw new IllegalArgumentException("Expected org.eclipse.aether.DefaultRepositorySystemSession, not " + session.getClass());
    }

    DefaultRepositorySystemSession defaultSession = ((DefaultRepositorySystemSession) session);
    try {
      defaultSession.setConfigProperty("aether.artifactResolver.snapshotNormalization", "false");
    } catch (Exception e) {
      try {
        Field readOnly = DefaultRepositorySystemSession.class.getDeclaredField("readOnly");
        readOnly.setAccessible(true);
        readOnly.set(session, false);

        try {
          defaultSession.setConfigProperty("aether.artifactResolver.snapshotNormalization", "false");
        } finally {
          readOnly.set(session, true);
        }
